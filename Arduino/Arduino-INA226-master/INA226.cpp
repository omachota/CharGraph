...
void INA226::waitForConversion()
{
	while (bitRead(getMaskEnable(), 3) == 0)
	{

	}
}

float INA226::readBusPower(bool wait = false)
{
	ina226_mode_t mode = getMode();

	if (wait && (mode == INA226_MODE_BUS_TRIG || mode == INA226_MODE_SHUNT_TRIG || mode == INA226_MODE_SHUNT_BUS_TRIG)) {
		int16_t configRegister = readRegister16(INA226_REG_CONFIG);
		writeRegister16(INA226_REG_CONFIG, configRegister);
		waitForConversion();
	}

	float value = readRegister16(INA226_REG_POWER) * powerLSB;
	if (!wait && (mode == INA226_MODE_BUS_TRIG || mode == INA226_MODE_SHUNT_TRIG || mode == INA226_MODE_SHUNT_BUS_TRIG)) {
		int16_t configRegister = readRegister16(INA226_REG_CONFIG);
		writeRegister16(INA226_REG_CONFIG, configRegister);
	}
...
float INA226::readShuntCurrent(bool wait = false)
{
	ina226_mode_t mode = getMode();

	if (wait && (mode == INA226_MODE_SHUNT_TRIG || mode == INA226_MODE_SHUNT_BUS_TRIG)) {
		int16_t configRegister = readRegister16(INA226_REG_CONFIG);
		writeRegister16(INA226_REG_CONFIG, configRegister);
		waitForConversion();
	}

	float value = readRegister16(INA226_REG_CURRENT) * currentLSB;
	if (!wait && (mode == INA226_MODE_SHUNT_TRIG || mode == INA226_MODE_SHUNT_BUS_TRIG)) {
		int16_t configRegister = readRegister16(INA226_REG_CONFIG);
		writeRegister16(INA226_REG_CONFIG, configRegister);
	}
...
float INA226::readShuntVoltage(bool wait = false)
{
	ina226_mode_t mode = getMode();
	...
	float voltage = readRegister16(INA226_REG_SHUNTVOLTAGE) * 0.0000025;
	if (!wait && (mode == INA226_MODE_SHUNT_TRIG || mode == INA226_MODE_SHUNT_BUS_TRIG)) {
		int16_t configRegister = readRegister16(INA226_REG_CONFIG);
		writeRegister16(INA226_REG_CONFIG, configRegister);
	}
...
float INA226::readBusVoltage(bool wait = false)
{
	ina226_mode_t mode = getMode();

	if (wait && (mode == INA226_MODE_BUS_TRIG || mode == INA226_MODE_SHUNT_BUS_TRIG)) {
		int16_t configRegister = readRegister16(INA226_REG_CONFIG);
		writeRegister16(INA226_REG_CONFIG, configRegister);
		waitForConversion();
	}

	float voltage = readRegister16(INA226_REG_BUSVOLTAGE) * 0.00125;
	if (!wait && (mode == INA226_MODE_BUS_TRIG || mode == INA226_MODE_SHUNT_BUS_TRIG)) {
		int16_t configRegister = readRegister16(INA226_REG_CONFIG);
		writeRegister16(INA226_REG_CONFIG, configRegister);
	}
